...
class EmbedPostion(nn.Sequential):
    def __init__(
        ...
        layers = nn.Sequential(
            ...
            qkmodels.PositionalEncoding(
                embed_dim, dropout, max_len=window_size),
        )
...
class WestERT(nn.Module):
    def __init__(
        ...
        self.former = nn.TransformerEncoder(
...
class WestGPT(WestERT):
    def __init__(self, input_dim, embed_dim, output_dim, window_size=1024, num_heads=8, num_layers=2, dropout_rate=0.1, noise_ratio=0.1, **kwargs):
        ...
        self.former = nn.TransformerDecoder(
    ...
    def forward(self, enc_inputs):
        ...
        enc_outputs = self.former(enc_inputs, 
                                  enc_inputs, 
                                  causal_mask, 
                                  causal_mask, 
                                  tgt_is_causal=True,
...
if __name__ == "__main__":
    ...
    model = WestGPT(input_dim, embed_dim, output_dim,
                    window_size=int(5 * 10 ** 3))
